...
library(tidyverse)
```
If you are unfamiliar with my_gapminder, we can view a description of the data using:
```{r, eval = FALSE}
head(my_gapminder)
```
# Fitting a t-test
...
### Test 1: 
  for the two-sided test, since the p-value two_sided_p is greater than the cut-off value of α =0.05,
